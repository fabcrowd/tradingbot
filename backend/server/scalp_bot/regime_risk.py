...
def regime_risk_on_triggers(iv: IndicatorValues, cfg: "ScalpBotConfig") -> list[str]:
    ...
    if not bool(getattr(cfg, "regime_risk_on_enabled", True)):
        return []
    reasons: list[str] = []
    vm = float(getattr(cfg, "regime_volume_spike_mult", 3.0))
    ...
    rsi_os = float(getattr(cfg, "regime_rsi_oversold", 20.0))
    rsi_ob = float(getattr(cfg, "regime_rsi_overbought", 80.0))
    if rsi_os > 0.0 and iv.rsi <= rsi_os:
        reasons.append("rsi_oversold")
    if rsi_ob > 0.0 and iv.rsi >= rsi_ob:
...
def regime_risk_on_triggers_live(
    ...
    *,
    ...
    # RSI extremes on last closed bar — regime is already stressed before next close.
    rsi_os = float(getattr(cfg, "regime_rsi_oversold", 20.0))
    rsi_ob = float(getattr(cfg, "regime_rsi_overbought", 80.0))
    if rsi_os > 0.0 and iv.rsi <= rsi_os:
        reasons.append("live_rsi_oversold")
    if rsi_ob > 0.0 and iv.rsi >= rsi_ob:
        reasons.append("live_rsi_overbought")
    return reasons

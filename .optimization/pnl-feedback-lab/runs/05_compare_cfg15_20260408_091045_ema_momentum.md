# Compare — run `agent_cfg15_20260408_091045_ema_momentum`
...
| Field | Value |
|-------|-------|
| Command | `run_multiwindow_lab config_only; compare --focus-strategy ema_momentum` |
...
**Focus mode (`--focus-strategy` override): `ema_momentum`** (operating interval = `config_interval_m` rows only)

| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | 1683.568006 | 21 | 1.7792 |
| mid | 425.909137 | 22 | 1.1669 |
| late | 3802.627136 | 24 | 2.4328 |

**Weakest window (focus):** `mid` (total_pnl=425.909137). **Thin sample (trades<5):** none.
...
| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | 2.081414 | 24 | 1.5460 |
| mid | 0.118363 | 22 | 1.0326 |
| late | 3.84071 | 21 | 1.9673 |

**Weakest window (focus):** `mid` (total_pnl=0.118363). **Thin sample (trades<5):** none.
...
| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | 0.027857 | 25 | 1.4696 |
| mid | -0.019632 | 25 | 0.7059 |
| late | -0.007124 | 24 | 0.9141 |

**Weakest window (focus):** `mid` (total_pnl=-0.019632). **Thin sample (trades<5):** none.
...
#### BTC_USD — `ema_momentum`

| Check | Result |
|-------|--------|
| G1 (≥5 trades per window) | **PASS** — 3/3 windows |
| G3 (profit factor ≥ 1 where finite) | **PASS** — 3/3 windows with finite PF |
| RULE C-style (≥2/3 windows with total_pnl > 0) | **PASS** — 3/3 positive |

#### SOL_USD — `ema_momentum`
...
1. Windows with PF < 1: mid, late — review mode or gates before scaling live.
2. Negative or flat windows on focus mode: mid, late.

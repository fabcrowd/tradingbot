# Compare — run `agent_cfg15_20260408_091045_rsi_reversion`
...
| Field | Value |
|-------|-------|
| Command | `run_multiwindow_lab config_only; compare --focus-strategy rsi_reversion` |
...
**Focus mode (`--focus-strategy` override): `rsi_reversion`** (operating interval = `config_interval_m` rows only)

| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | -1215.252284 | 5 | 0.1769 |
| mid | 726.661622 | 2 | — |
| late | -1100.142165 | 6 | 0.4641 |

**Weakest window (focus):** `early` (total_pnl=-1215.252284). **Thin sample (trades<5):** mid.
...
| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | -0.660954 | 2 | 0.3951 |
| mid | -0.32764 | 1 | 0.0000 |
| late | -5.557526 | 8 | 0.1038 |

**Weakest window (focus):** `late` (total_pnl=-5.557526). **Thin sample (trades<5):** early, mid.
...
| Window | total_pnl | trades | PF |
|--------|-----------|--------|-----|
| early | -0.014563 | 3 | 0.2765 |
| mid | 0.006016 | 1 | — |
| late | -0.020357 | 5 | 0.4951 |

**Weakest window (focus):** `late` (total_pnl=-0.020357). **Thin sample (trades<5):** early, mid.
...
#### BTC_USD — `rsi_reversion`

| Check | Result |
|-------|--------|
| G1 (≥5 trades per window) | **FAIL** — 2/3 windows |
| G3 (profit factor ≥ 1 where finite) | **FAIL** — 0/2 windows with finite PF |
| RULE C-style (≥2/3 windows with total_pnl > 0) | **FAIL** — 1/3 positive |

#### SOL_USD — `rsi_reversion`

| Check | Result |
|-------|--------|
| G1 (≥5 trades per window) | **FAIL** — 1/3 windows |
...
1. Raise sample bar (e.g. `wfo_min_trades` / longer history) — thin windows: mid.
2. Windows with PF < 1: early, late — review mode or gates before scaling live.
3. Negative or flat windows on focus mode: early, late.
4. Raise sample bar (e.g. `wfo_min_trades` / longer history) — thin windows: early, mid.
5. Windows with PF < 1: early, mid, late — review mode or gates before scaling live.
6. Negative or flat windows on focus mode: early, mid, late.

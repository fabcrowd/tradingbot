...
def min_bars_ready_for_mode(mode: str, cfg: Any) -> int:
    ...
    if m == "supertrend":
        ...
        return w + 2  # vec clears ``[:warmup+1]``; first signal at ``warmup+1``
    if m == "squeeze_momentum":
        w = max(
            int(getattr(cfg, "squeeze_bb_period", 20)),
            int(getattr(cfg, "atr_period", 14)),
            int(getattr(cfg, "squeeze_mom_period", 12)),
        )
        return w + 2  # vec ``[:warmup]`` with warmup = max(...) + 1
    if m == "qqe_mod":
        rsi_period = int(getattr(cfg, "qqe_rsi_period", 14))
        wilders_period = rsi_period * 2 - 1
        w = max(
            rsi_period,
            int(getattr(cfg, "qqe_smoothing", 5)),
            wilders_period,
        )
        return w + 2
    if m == "utbot_alert":
        w = int(getattr(cfg, "utbot_atr_period", 10))
        return w + 2  # vec clears ``[:warmup+1]``
    if m == "hull_suite":
        ...
        return w + 1  # vec ``long_mask[:warmup]`` with that warmup sum
    if m == "sar_chop":
